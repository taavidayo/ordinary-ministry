import { PageBlock, PageSection } from "@/lib/page-blocks"
...
function renderBlockContent(block: PageBlock) {
  switch (block.type) {
    ...
    case "text": {
      ...
      if (block.sectionBg) s.backgroundColor = block.sectionBg
      if (block.rotate) s.transform = `rotate(${block.rotate}deg)`
      const contentStyle: React.CSSProperties = {}
      if (block.letterSpacing) contentStyle.letterSpacing = block.letterSpacing
      return (
        <section style={s} className="py-8 px-6 h-full">
          <div className="prose-content" style={contentStyle} dangerouslySetInnerHTML={{ __html: block.html }} />
        </section>
      )
    }
      ...
      return (
        <section style={s} className="py-6 px-6 h-full">
          <div className={`${widthClass} ${alignClass}`}>
            {block.src
              ? <img src={block.src} alt={block.alt || ""} className={`w-full h-auto ${block.rounded ? "rounded-xl" : ""}`} /> // eslint-disable-line @next/next/no-img-element
    ...
    case "button": {
      ...
      return (
        <section style={s} className={`py-6 px-6 h-full ${alignClass}`}>
          <a href={block.href || "/"} className={`${variantClass} ${sizeClass} rounded-lg font-semibold inline-block transition-colors`}>
            {block.text}
          </a>
        </section>
      )
    ...
    case "divider": {
      const s = block.sectionBg ? { backgroundColor: block.sectionBg } : undefined
      return (
        <div style={s} className="py-4 px-6 h-full flex items-center">
            ...
            {block.cards.map((card, i) => (
              <div key={i} className="rounded-xl border p-6 bg-card">
                ...
                {card.link && card.linkText && (
                  <a href={card.link} className="text-primary text-sm font-medium mt-3 inline-block hover:underline">{card.linkText} &rarr;</a>
                )}
              </div>
            ))}
          </div>
    ...
    case "events": {
      const s = block.sectionBg ? { backgroundColor: block.sectionBg } : undefined
      return (
        <section style={s} className="py-8 px-6 h-full flex items-center justify-center bg-gray-50 border-2 border-dashed border-gray-200 rounded">
          <div className="text-center text-muted-foreground">
            <p className="text-3xl mb-2">📅</p>
            <p className="font-medium">{block.heading || "Events"}</p>
            <p className="text-xs mt-1 opacity-60">Upcoming events — live on the public site</p>
          </div>
        </section>
      )
    }
    ...
    case "give": {
      ...
      return (
        <section style={s} className="py-8 px-6 h-full flex items-center justify-center bg-gray-50 border-2 border-dashed border-gray-200 rounded">
          <div className="text-center text-muted-foreground">
            <p className="text-3xl mb-2">💝</p>
            <p className="font-medium">{block.heading || "Give"}</p>
            <p className="text-xs mt-1 opacity-60">Stripe giving form — live on the public site</p>
          </div>
        </section>
      )
    }
    ...
    <div style={containerStyle}>
      ...
      {ytId && (
        <div style={{ position: "absolute", inset: 0, zIndex: 0, overflow: "hidden", pointerEvents: "none" }}>
          <iframe
            src={`https://www.youtube.com/embed/${ytId}?autoplay=1&mute=1&loop=1&playlist=${ytId}&controls=0&disablekb=1&playsinline=1&rel=0`}
      ...
      {/* Overlay */}
      {section.bgOverlay && (
        <div style={{ position: "absolute", inset: 0, zIndex: 1, backgroundColor: section.bgOverlay }} />
      )}
      {/* Content */}
...
export default function PageBlockRenderer({
  ...
  // Legacy flat blocks rendering
  if (!blocks?.length) return null
  return renderSectionBlocks(blocks)
}
